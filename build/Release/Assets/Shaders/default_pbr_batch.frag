...
struct MaterialTextures
{
	...
	sampler2DArray metallicAndRoughnessMap;
	sampler2DArray aoMap;
...
vec2 ParallaxMapping(vec2 texCoords, vec3 viewDir)
{
	...
	vec2 P = viewDir.xy / viewDir.z * materials[fs_in.materialIndex].heightScale;
	vec2 deltaTexCoords = P / numLayers;

	// get initial values
	vec2 currentTexCoords = texCoords;
	float currentDepthMapValue = texture(materialTextures.heightMap, vec3(currentTexCoords, fs_in.materialIndex)).r;
	...
	while (currentLayerDepth < currentDepthMapValue)
	{
		...
		currentDepthMapValue = texture(materialTextures.heightMap, vec3(currentTexCoords, fs_in.materialIndex)).r;
	...
	float beforeDepth = texture(materialTextures.heightMap, vec3(prevTexCoords, fs_in.materialIndex)).r - currentLayerDepth + layerDepth;
...
void main()
{
	...
	// if (materials[fs_in.materialIndex].heightScale > 0.0)
	//{
	...
	// }
	//  properties
	vec3 N = normalize(texture(materialTextures.normalMap, vec3(texCoords, fs_in.materialIndex)).rgb * 2.0 - 1.0);

	vec3 albedo = pow(texture(materialTextures.albedoMap, vec3(texCoords, fs_in.materialIndex)).rgb * materials[fs_in.materialIndex].albedo, vec3(2.2));
	float metallic = texture(materialTextures.metallicAndRoughnessMap, vec3(texCoords, fs_in.materialIndex)).b * materials[fs_in.materialIndex].metallic;
	float roughness = texture(materialTextures.metallicAndRoughnessMap, vec3(texCoords, fs_in.materialIndex)).g * materials[fs_in.materialIndex].roughness;
	float ao = texture(materialTextures.aoMap, vec3(texCoords, fs_in.materialIndex)).r;
